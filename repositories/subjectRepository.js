...
async function findAll() {
    const [rows] = await pool.query("SELECT id, subject_code, subject_title, " +
        "credit FROM subjects");
    return rows;
}
...
async function findById(id) {
    ...
    subject = {
        id : rows[0].id, code:rows[0].subject_code,
...
async function save(subject) {
    const [result] = await pool.query("INSERT INTO subjects (subject_code, " +
...
async function update(subject) {
    const [result] = await pool.query("UPDATE subjects SET subject_code = ?, " +
